...
def orbitAroundPoint(ox, oy, deg):
  ...
  if(SCENE_STEP<2):
    setCamera(qx, qz, 0, 25, 0, deg) #deg
  else:
    setCamera(qx, qz, 2, 20, 0, deg) #deg
...
def updateTerrainSand():
  ...
  setCamera(getCameraPX(),getCameraPY(), sin*4+3, 20,0,getCameraRZ())
...
def updateTerrainNight():
  ...
  if(player_y >= 14.6 and getObjectPZ(player_name)>0.5):
    ...
    setCamera(getCameraPX(),getCameraPY(), 2.06+2, 0,0,getCameraRZ())
  ...
  # Stairs
    ...
    setCamera(getCameraPX(),getCameraPY(), pz+2, 0,0,getCameraRZ())
...
def update():
  ...
  if(SCENE_STEP == -1):
    ...
    disableObject("coll_camel")
    disableObject("coll_tornado")
  ...
  if(SCENE_STEP == 6):
    ...
    if(getDistanceObj(player_name, "Ghost_White") < 0.6):
      ...
      TPVController = False
      setCamera(getObjectPX("Ghost_White")+1.3,getObjectPY("Ghost_White")-1,1.2,7,0,-70)
  ...
  if(SCENE_STEP == 12):
    disableObject("lives")
    setCamera(-2,6.5+(getObjectPY(boss[0])-17),4+getObjectPZ(boss[0]),5,0,15)
